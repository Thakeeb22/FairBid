...
export type AuctionStatus = "active" | "reveal" | "finalized"; // frontend mapping

export interface Auction {
  _id: string;
  id?: string; // optional if frontend uses `id` instead of _id
  title: string;
...
// -----------------------------
// Helper: Map backend -> frontend status
// -----------------------------
...
const AuctionCard: React.FC<AuctionCardProps> = ({ auction, index = 0 }) => {
  ...
  // Ensure deadlines are Date objects
  ...
  const getCtaConfig = () => {
    switch (auction.status) {
      case "commit":
        return { label: "Place Sealed Bid", route: `/commit/${auction._id}`, variant: "gold" };
      case "reveal":
        return { label: "Reveal Bid", route: `/reveal/${auction._id}`, variant: "outline" };
      case "finalized":
        return { label: "View Results", route: `/finalized/${auction._id}`, variant: "ghost" };
    }
  };
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="font-heading font-bold text-gold">{auction.startingPrice} STRK</p>
          </div>
          {auction.highestBid ? (
